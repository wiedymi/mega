import File, { LABEL_NAMES } from './file'
...
import { detectSize, streamToCb, promisify } from './util'
...
class MutableFile extends File {
  ...
  async mkdirAsync () {
    return promisify(this.mkdir.bind(this)).call(this, ...arguments)
  }

  async uploadAsync () {
    return promisify(this.upload.bind(this)).call(this, ...arguments)
  }

  async deleteAsync () {
    return promisify(this.delete.bind(this)).call(this, ...arguments)
  }

  async moveToAsync () {
    return promisify(this.moveTo.bind(this)).call(this, ...arguments)
  }

  async renameAsync () {
    return promisify(this.rename.bind(this)).call(this, ...arguments)
  }

  async setLabelAsync () {
    return promisify(this.setLabel.bind(this)).call(this, ...arguments)
  }

  async setFavoriteAsync () {
    return promisify(this.setFavorite.bind(this)).call(this, ...arguments)
  }

  async linkAsync () {
    return promisify(this.link.bind(this)).call(this, ...arguments)
  }

  async shareFolderAsync () {
    return promisify(this.shareFolder.bind(this)).call(this, ...arguments)
  }

  async unshareFolderAsync () {
    return promisify(this.unshareFolder.bind(this)).call(this, ...arguments)
  }

  async importFileAsync () {
    return promisify(this.importFile.bind(this)).call(this, ...arguments)
  }
  ...
  _uploadWithSize (stream, size, source, pause, type, opt, cb) {
    const ssl = process.env.IS_BROWSER_BUILD || opt.forceHttps ? 2 : 0
    const getUrlRequest =
      type === 0
    ...
    const sendChunk = () => {
      ...
      httpreq.on('response', (response) => {
        ...
        stream.emit(
          'error',
          Error('MEGA returned a ' + response.statusCode + ' status code')
        )
      })
