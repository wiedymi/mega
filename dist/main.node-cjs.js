...
class File extends events.EventEmitter {
  ...
  link(options, cb) {
    ...
    let url$$1 = `https://mega.nz/${this.directory ? 'folder' : 'file'}/${this.downloadId}`;
  ...
  static fromURL(opt) {
    ...
    if (url$$1.path.match(/\/(file|folder)\//) !== null) {
      // new format
      const split = url$$1.hash.substr(1).split('/file/');
      const fileHandler = url$$1.path.substring(url$$1.path.lastIndexOf('/') + 1, url$$1.path.length + 1);
...
class MutableFile extends File {
  ...
  link(options, cb) {
    ...
    this.api.request({
      ...
      if (err) return cb(err);
      let url$$1 = `https://mega.nz/${folderKey ? 'folder' : 'file'}/${id}`;
  ...
  importFile(sharedFile, cb) {
    ...
    if (!sharedFile.key) {
      return cb(Error("Can't import files without encryption keys"));
